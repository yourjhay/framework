...
use Simple\Validation\ArrayHelpers;
use Simple\Validation\EnvHelpers;
use Exception;
use DateTime;
...
class GUMP
{
        ...
        foreach ($fields as $field) {
            ...
            if (is_string($value)) {
                ...
                if (function_exists('iconv') && function_exists('mb_detect_encoding') && $utf8_encode) {
                    $current_encoding = mb_detect_encoding($value, null, true);
    ...
    protected function filter_slug($value, array $params = [])
    {
        ...
        return mb_strtolower(trim(preg_replace('/[\s-]+/', $delimiter, preg_replace('/[^A-Za-z0-9-]+/', $delimiter, preg_replace('/[&]/', 'and', preg_replace('/[\']/', '', iconv('UTF-8', 'ASCII//TRANSLIT', $value))))), $delimiter), 'UTF-8');
    }
    ...
    {
        $value = mb_strtolower(trim($input[$field]), 'UTF-8');
    ...
    {
        $url = parse_url(mb_strtolower($value, 'UTF-8'));
